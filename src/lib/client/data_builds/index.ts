...
export async function buildHistoricalData({
  ...
  yearFrom,
  yearTo
}: { found: string; monthFrom: number; monthTo: number; yearFrom: number; yearTo: number }) {
  const isRangeInverted = yearFrom > yearTo || (yearFrom === yearTo && monthFrom > monthTo)
  ...
  const labels: string[] = []
  if (yearFrom === yearTo) {
    const months = [...Array(normalizedDates.monthTo - normalizedDates.monthFrom + 1).keys()].map(
      (i) => normalizedDates.monthFrom + i
    )
    for (const month of months) labels.push(`${yearFrom}-${month.toString().padStart(2, '0')}`)
  } else {
    const months = [...Array(12).keys()].map((i) => 1 + i)
    const years = [...Array(normalizedDates.yearTo - normalizedDates.yearFrom + 1).keys()].map(
      (i) => normalizedDates.yearFrom + i
    )
    years.forEach((year, index) => {
      const isFirstYear = index === 0
      const isLastYear = index === years.length - 1
      if (isFirstYear) {
        const months = [...Array(12 - normalizedDates.monthFrom + 1).keys()].map((i) => i + normalizedDates.monthFrom)
        for (const month of months) labels.push(`${year}-${month.toString().padStart(2, '0')}`)
        return
      }
      if (isLastYear) {
        const months = [...Array(normalizedDates.monthTo).keys()].map((i) => i + 1)
        for (const month of months) labels.push(`${year}-${month.toString().padStart(2, '0')}`)
        return
      }
      for (const month of months) labels.push(`${year}-${month.toString().padStart(2, '0')}`)
  ...
  const csvString = await fetchResource<string>({
    cacheName: HISTORICAL_DATA.cacheName,
    route: HISTORICAL_DATA.route,
    isJson: false
  })
  const content = csvString.split('\n')
  if (!content) throw new Error('Error fetching csv historical data')
  const preDatasets: {
    [key: string]: Array<number | null>
  } = {}
  for (const line of content) {
    const [afpName, month, year, rowFound, rentability] = line.split(',')
  ...
  const date2 = yearMonthToDate(normalizedDates.yearTo.toString(), normalizedDates.monthTo.toString().padStart(2, '0'))
  ...
  for (const row of content) {
    ...
    if (date < fromDate || date > toDate || rowFound !== found || !(afpName in AFPS) || afpsOutOfPeriod.has(afpName))
      continue

...
export function getColor(
  ...
  const condition = lessIsBetter
    ? actualValue < avarageValue
    : actualValue > avarageValue
...
export function getRetability(rentabilities: number[]) {
  ...
  for (const rentability of rentabilities)
    acc = acc.times(new Decimal(rentability).div(100).add(1))
  const rentabilityPercent = acc.minus(1).times(100)
  return rentabilityPercent.toDecimalPlaces(3).toNumber()
}

export function calculateAccumulatedRentability(
  rentabilities: Array<number | null>
): Array<number | null> {
  ...
  for (const rentability of rentabilities) {
    ...
    accumulatedValue = accumulatedValue.plus(
      accumulatedValue
        .times(decimalRentability.div(100))
        .plus(decimalRentability)
    )
    accumulatedRentabilities.push(
      accumulatedValue.toDecimalPlaces(3).toNumber()
    )
  }

...
import { buildHistoricalData } from '@/lib/client/data_builds'
import { For, type JSX, Show, createSignal, onMount } from 'solid-js'

export default function BestAfpByPeriodSection() {
  ...
  const buildTableAndChart = ({
    ...
    buildHistoricalData({
      ...
      const filteredDatasets = datasets.filter(({ data }) => data.every((d) => d != null))
      tableData.length ? setBestAfp(tableData[0][0]) : setBestAfp('')
      setAfpsOutOfPeriod([...afpsOutOfPeriod])
      bestAfpsPeriodTable.setAttribute('data-table-rows', JSON.stringify(tableData))
      historicalChart.updateChart({ newDatasets: filteredDatasets, newLabels: labels })
    })
  }
  onMount(() => {
    const now = new Date()
  ...
  return (
    <section>
      ...
      <article class="chart-container">
        ...
        <historical-chart ref={historicalChart} style={{ display: bestAfp() ? 'block' : 'none' }} />

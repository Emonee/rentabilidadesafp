...
export class HistoricalChart extends BaseChart {
  ...
  generateLabels() {
    ...
    const years = [...Array(yearTo - yearFrom + 1).keys()].map(
      (i) => yearFrom + i
    )
    years.forEach((year, index) => {
      ...
      if (isFirstYear) {
        const months = [...Array(12 - monthFrom + 1).keys()].map(
          (i) => i + monthFrom
        )
        for (const month of months)
  ...
  generateDataset({ labels }: { labels: string[] }): ChartDataset[] {
    ...
    for (const line of this.historicalData) {
      const [afpName, month, year, found, rentability] = line

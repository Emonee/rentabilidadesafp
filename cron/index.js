...
const validAfps = ['CAPITAL', 'CUPRUM', 'HABITAT', 'MODELO', 'PLANVITAL', 'PROVIDA', 'UNO']
...
const tables = $('table.table.table-striped.table-hover.table-bordered.table-condensed')
...
tables.slice(1).each((index, table) => {
  ...
  trs.slice(4).each((_, tr) => {
    ...
    try {
      monthRentability = Number.parseFloat($(tds[1]).text().replace('%', '').replace(',', '.'))
    } catch (e) {}

    try {
      ytd = Number.parseFloat($(tds[2]).text().replace('%', '').replace(',', '.'))
    } catch (e) {}
...
try {
  ...
  await Promise.all([
    ...
    writeFile(YTD_12_MONTHS_FILE_ROUTE, `${JSON.stringify(jsonData, undefined, 2)}\n`)
  ])
  execSync('git add .')
